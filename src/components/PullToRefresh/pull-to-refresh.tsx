...
import { getRect } from '@antmjs/vantui/es/common/utils'
...
export default function PullToRefresh(props: any) {
  ...
  const onTouchMove = useCallback(
    function (event) {
      if (!disable && Date.now() - touch.time > TOUCH_MIN_TIME && (status === 'pull' || status === 'release')) {
  ...
  useEffect(() => {
    setTimeout(() => {
      getRect(null, `.${clsPrefix}-status${componentIndex}`).then((res: any) => {
  ...
  return (
    <View onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} className={`${clsPrefix}`}>
      <View
